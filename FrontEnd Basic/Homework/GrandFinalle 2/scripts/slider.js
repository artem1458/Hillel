(function() {
  ...
  function controlSlideTo() {
    ...
    slideTo(
      sliderNode,
      slidesNode,
      sliderNodeOffsetWidth,
      parseInt(this.getAttribute('data-slide-to'), 10),
    );
  }
  ...
  function createControl(sliderNode) {
    function createControlDots() {
      const sliderSlideCollection = sliderNode.querySelectorAll(
        '.slider__slide',
      );
      ...
      if (sliderNode.classList.contains('slider--preview')) {
        const slidesPreviewCollection = sliderNode.querySelectorAll(
          '.slider__control-dot-preview-img',
        );
        let paths = [];
    ...
    function createControlArrows() {
      ...
      btnPrev.addEventListener(
        'click',
        changeSlide.bind(null, ['prev', sliderNode]),
      );
      btnNext.addEventListener(
        'click',
        changeSlide.bind(null, ['next', sliderNode]),
      );
  ...
  function changeSlide(...args) {
    ...
    const slidesNode = sliderNode.querySelector('.slider__slides');
    const slidesCount =
      slidesNode.querySelectorAll('.slider__slide').length - 1;
    const currentSlide = parseInt(
      sliderNode.getAttribute('data-current-slide'),
      10,
    );
  ...
  function addInterval(sliderNode) {
    ...
    const interval =
      parseFloat(sliderNode.getAttribute('data-slider-change-interval')) * 1000;
    if (interval > 0) {
      ...
      console.log(
        RangeError(
          'Warning: value of "data-slider-change-interval" must be above 0',
        ),
      );
    }
  }
  ...
  function startSlide(sliderNode) {
    if (!sliderNode.getAttribute('data-current-slide')) {
      ...
      slideTo(
        sliderNode,
        sliderNode.querySelector('.slider__slides'),
        sliderNode.offsetWidth,
        0,
      );
      return;
    }
